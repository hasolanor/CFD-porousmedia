...
<figure>
  ...
  <figcaption>Fig.1 - 2D Geometry used in this CFD study. At the left, the dimensions of the physical domain in meters. At the right, the definition of the boudaries and two axes used in the study.</figcaption>
  <br><br>
</figure>
...
Regarding the boundary conditions, a fixed inlet-velocity and pressure zero grandient are considered as inlet boundary conditions.
...
Instead, at the outlet boundary, constant pressure and velocity zero gradient, corresponding to an outlow-type condition. 
...
For the grains boundary, a model for retention is used to represent surface phenomena. **Bueno et. al. (2020)** propose an schema based on an Langmuir-type retention model, in which attachment onto the surface depends on the chemical species concentration in the fluid and the retained concentration. So, this boundary condition can be expressed, for an irreversible dynamics, as follow:
...
To solve the mathematical formulation, both problems are solved separately. This is possible due to the time required to reach the steady state in the incompressible flow is enough low respect to the species transport if the Reynolds and Ruark numbers are in the typical values used for geological porous media (**Bueno et. al., 2020**). Both problems, the incompressible flow and species transport ones, are solved using OpenFOAM (**OpenFOAM, 2018a, 2018b**), a widely recognised open source package for CFD applications with a Finite Volume-based framework (**Moukalled, 2016**).
...
The incompressible flow simulation is run using the input data presented in Table 5. This table contains the dimesionless numbers obtained from the input variables.
